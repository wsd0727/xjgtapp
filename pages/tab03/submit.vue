<template>
	...
	<view class="">
		<view class="card-list-wrapper">
			<form>
				<view class="cu-form-group text-m" :class="formActiveIndex==index?   'solid-bottom-blue':'' " v-if='item.ISVISIBLE==1'
				 v-for="(item,index) in configForm">
					...
					<view class="cu-form-input text-m" v-if="item.CTRLTYPE == 'ExTextBox' ">
						<input type="text" class="text-m" @focus="setActive(index)" v-if="item.FIELDTYPE == 'varchar'|| item.FIELDTYPE == 'nvarchar'"
						 v-model="formValue[item.FIELDNAME]" value="" :placeholder="'请输入'+ item.DISPLAYNAME" />
						<input type="number" class="text-m" @focus="setActive(index)" v-if="item.FIELDTYPE == 'number' || item.FIELDTYPE == 'decimal'"
						 v-model="formValue[item.FIELDNAME]" value="" :placeholder="'请输入'+ item.DISPLAYNAME" />
					</view>
					...
					<view class="cu-form-val text-m text-gray" v-if="item.CTRLTYPE == 'ExReadCard' ">
						{{ formValue[item.FIELDNAME] || '' }}
					</view>

					<view class="flex-row-between text-m myrow" v-if="item.CTRLTYPE == 'ExEnum' ">
						<picker @change="bindPickerChange($event,item.FIELDNAME,index)" :value="formValue[item.FIELDNAME+'VI']" :range="formValue[item.FIELDNAME+index]">
...
<script>
	...
	let _self, ajaxurl, baseUrl, myToken, voluationJson, userId, token ,searchDataConfig=[];
	let isBack = false;
	export default {
		data() {
			return {
		...
		onLoad(params) {
			...
			this.uniSkip.getParams(function(res) {
				...
				if(res.CONFIGMODEL.length){
					if(res.CONFIGMODEL[0].BILLNO=='IsEncryptPIDTel' && res.CONFIGMODEL[0].VALUE==1 ){
						_self.isAes = true
					}
				}
		...
		methods: {
			...
			submitForm() {
				let data = JSON.parse(JSON.stringify(_self.formValue)) ;
				data.userId = userId
				...
				if(_self.isAes){
					if(data.DRID ){
						data.DRID = this.$utils.aesJmEncrypt(data.DRID)
					}
					if(data.DRTEL){
						data.DRTEL = this.$utils.aesJmEncrypt(data.DRTEL)
					}
					if(data.DRJSID){
						data.DRJSID = this.$utils.aesJmEncrypt(data.DRJSID)
					}
				}
				// console.log(data);
				// this.$utils.aesJmEncrypt(this.password)
				if (!checkRes) {
					return uni.showToast({
				...
				_self.$axios(params)
					.then(res => {
						...
						_self.initForm();
						if (isBack) {
							uni.$emit('getSelectItem', res.Result[0]);
							setTimeout(()=>{
								uni.navigateBack({
									delta: 2
								});
